...
import { appShell, escapeHtml, navigate, progressBar } from '../features/ecocrew/page-utils.js';
...
function membershipForm(mode) {
  ...
  return `
    <form class="ecocrew-crew-form" data-crew-form="${mode}">
      ...
      <label>${isJoining ? 'Invite code' : 'Crew name'}<input name="value" type="text" ${isJoining ? 'placeholder="e.g. GLASS-GUARDIANS"' : 'placeholder="e.g. Green Dream Team"'} minlength="3" maxlength="40" required></label>
...
function emptyCrew() {
  return `
    <section class="ecocrew-crew-section" aria-labelledby="your-crew-title">
      ...
      <div class="ecocrew-crew-empty"><span aria-hidden="true"><i class="bi bi-people"></i></span><strong>You have not joined a crew yet</strong><p>Crews make every good choice count toward a shared mission and streak.</p></div>
    </section>`;
}
...
function bindShareMenu(page, membership) {
  ...
  const link = `${window.location.origin}${window.location.pathname}#join/${encodeURIComponent(membership.inviteCode)}`;
...
export function renderFriendsPage() {
  ...
  const page = appShell(membership ? escapeHtml(membership.crewName) : 'Crew hub', 'Crews', `
    <div data-crew-content>${membership ? '' : membershipActions()}${membership ? crewContent(state, membership) : emptyCrew()}</div>

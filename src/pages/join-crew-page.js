...
export function renderJoinCrewPage({ params = {}, session, navigate = defaultNavigate } = {}) {
  ...
  const page = standaloneShell(
    'Join the crew',
    'You’re invited',
    '<section class="ecocrew-card ecocrew-join-card" aria-labelledby="join-title"><div class="ecocrew-join-card__badge" aria-hidden="true">✦</div><p class="ecocrew-kicker">DEMO CREW INVITE</p><h2 id="join-title">Make one good choice together.</h2><p class="ecocrew-muted">You’ll join Glass Guardians and help turn today’s bottle check into shared progress.</p><div class="ecocrew-invite-code" aria-label="Invite code">' +
      escapeHtml(code || 'INVITE') +
      '</div><p class="ecocrew-form-error" data-join-error role="alert" hidden></p><button class="btn ecocrew-btn-primary w-100" type="button" data-join-action>Join Glass Guardians</button><p class="ecocrew-join-note" data-join-note>Sign in or create an account first. Your invite will stay with you.</p></section>',

...
function collection(items) {
  if (!items.length) return '<p class="ecocrew-muted">Cosmetics will appear here as your crew progresses.</p>';
  return items
    .map(
      (item) =>
        ...
        '</strong><small>' +
        (item.equipped ? 'Equipped' : item.unlocked ? 'Ready to equip' : escapeHtml(item.progress || 'Keep contributing')) +
        '</small>' +
        (item.unlocked && !item.equipped ? '<button class="btn btn-sm ecocrew-btn-secondary" type="button" data-equip="' + escapeHtml(item.id) + '">Equip</button>' : '') +
        '</article>',
    )
    .join('');
}
...
export function renderLeaderboardPage({ navigate = defaultNavigate } = {}) {
  const page = appShell(
    ...
    '<section class="ecocrew-league-summary" data-league-summary data-league-state="loading"><span aria-hidden="true">🌱</span><div><strong data-league-rank>Loading standings…</strong><p data-league-position>Every correct post helps your crew move up.</p></div></section>' +
      ...
      '<section class="ecocrew-card ecocrew-league-state" data-league-unranked hidden><span aria-hidden="true">🌱</span><p class="ecocrew-kicker">CREW NOT RANKED YET</p><h2 data-league-unranked-title>Your crew is not ranked yet.</h2><p class="ecocrew-muted" data-league-unranked-copy>Complete league activity with your crew to appear in the weekly standings.</p><button class="btn ecocrew-btn-secondary" type="button" data-action="crew">View crew</button></section>' +
  ...
  function renderLeagueOverview(overview = {}) {
    const state = leagueEligibility(overview);
    ...
    page.querySelector('[data-league-reset]').textContent = overview.resetLabel || 'Resets Monday at 00:00 SGT.';

    if (state === 'no_crew') {
      page.querySelector('[data-league-rank]').textContent = 'Not in a league';
      page.querySelector('[data-league-position]').textContent = 'Join a crew to enter the weekly standings.';
    ...
    if (state === 'unranked') {
      ...
      page.querySelector('[data-league-position]').textContent = crewName + ' will appear here once it has league activity.';
      page.querySelector('[data-league-unranked-title]').textContent = crewName + ' is not ranked yet.';
      page.querySelector('[data-league-unranked-copy]').textContent = 'Complete league activity with your crew to appear in the weekly standings.';
      page.querySelector('[data-league-points-label]').textContent = 'League points after first result';
      page.querySelector('[data-league-points]').textContent = '—';
      return;
    }

    page.querySelector('[data-league-rank]').textContent = own ? '#' + own.rank + ' of ' + rows.length : 'Ranked';
    ...
    page.querySelector('[data-league-points]').textContent = Number(overview.weeklyPoints || own?.score || 0).toLocaleString();
  ...
  function renderLeagueError(exception) {
    ...
    rankingsTarget.innerHTML = '<p class="ecocrew-form-error" role="alert">' + escapeHtml(exception.message || 'League data is unavailable.') + '</p>';
  }
  ...
  async function renderCosmetics() {
    try {
      ...
      page.querySelector('[data-cosmetics]').innerHTML = '<p class="ecocrew-form-error" role="alert">' + escapeHtml(exception.message || 'Cosmetics are unavailable.') + '</p>';
    }
  }

...
export const sortPage = () => ({
  title: 'Today’s task',
  content: `<section class="page-intro mb-4"><p class="text-success fw-semibold mb-1">Daily task</p><h1 class="display-6 fw-bold" data-task-prompt>Loading…</h1><p class="text-secondary">Upload a photo that proves you completed the task. The image is analyzed in memory and then discarded.</p></section><section class="surface-card card"><div class="card-body p-4"><form data-task-form><div class="mb-3"><label class="form-label" for="task-image">Task image</label><input class="form-control" id="task-image" name="image" type="file" accept="image/jpeg,image/png,image/webp" required><div class="form-text">JPEG, PNG, or WebP up to 10 MB.</div></div><p class="text-danger small d-none" data-form-error role="alert"></p><button class="btn btn-primary" type="submit">Submit task</button></form></div></section>`,

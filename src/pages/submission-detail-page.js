...
export function renderSubmissionDetailPage({ navigate = defaultNavigate } = {}) {
  const page = appShell(
    'Your result',
    'Post checked',
    '<section class="ecocrew-result" data-result-card><div class="ecocrew-result__burst" data-result-icon aria-hidden="true">✦</div><p class="ecocrew-kicker" data-result-kicker>RESULT</p><h2 data-result-title>Checking your post…</h2><p data-result-summary>We’re loading the disposal guidance.</p><p class="ecocrew-muted" data-result-task hidden></p></section>' +
      '<section class="ecocrew-card ecocrew-result-breakdown" data-result-breakdown hidden><div class="ecocrew-card__top"><div><p class="ecocrew-kicker">DISPOSAL GUIDE</p><h2 data-result-bin>—</h2></div><span data-result-confidence>—</span></div><details><summary>Why this result?</summary><p data-result-reason></p></details><div class="ecocrew-tip" data-result-tip hidden><strong>Prep tip</strong><span></span></div><div class="ecocrew-points" data-result-points></div></section>' +
    ...
    afterRender: async () => {
      ...
      if (!result) {
        ...
        page.querySelector('[data-result-summary]').textContent =
          'Create a post first, then come back to see its guidance.';
        return;
      }

      const classification = result.classification || result;
      const isTaskCompletion = Boolean(result.task || result.taskId);
      ...
      page.querySelector('[data-result-icon]').textContent = isCorrect
        ? '✓'
        : needsConfirmation
          ? '?'
          : '!';
      page.querySelector('[data-result-kicker]').textContent =
        isCorrect && isTaskCompletion
          ? 'TASK COMPLETE'
          : isCorrect
            ? 'CORRECT SORT'
            : needsConfirmation
              ? 'NEEDS A CLOSER LOOK'
              : 'TRY AGAIN NEXT TIME';
      page.querySelector('[data-result-title]').textContent =
        isCorrect && isTaskCompletion
          ? 'Great work!'
          : isCorrect
            ? 'Nice choice.'
            : needsConfirmation
              ...
              : 'That bin was not the best match.';
      page.querySelector('[data-result-summary]').textContent = needsConfirmation
        ...
        : isCorrect && isTaskCompletion
          ? 'Your evidence was verified and your progress has been updated.'
          : 'You chose ' +
            (result.userSelectedBin || 'a bin') +
            '. The recommended destination is shown below.';
      const taskSummary = page.querySelector('[data-result-task]');
      if (taskSummary && result.task?.prompt) {
        taskSummary.hidden = false;
        taskSummary.textContent = 'Daily task: ' + result.task.prompt;
      }
      ...
      page.querySelector('[data-result-reason]').textContent =
        ...
        result.reason ||
        'The server did not provide an additional explanation.';
